...
return ( <svg
   ...
   strokeLinecap="round"
   strokeLinejoin="round"
 > <path d="M12 20h9" /> <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L8 18l-4 4-1-4Z" /> </svg>
);
}
...
async function loadInitialProduct() {
  ...
  if (!companyId) {
    if (!cancelled) {
      setError("No authenticated company context is available.");
      setLoading(false);
    }
...
if (!canUpdate) {
  setUpdateError("You do not have permission to update products.");
  return;
}

if (!companyId) {
  setUpdateError("No authenticated company context is available.");
  return;
}
...
try {
  const request: ProductCreateRequest = {
    ...
    costPrice:
      editForm.costPrice === null || editForm.costPrice === undefined
        ? null
        : Number(editForm.costPrice),

    sellingPrice:
      editForm.sellingPrice === null || editForm.sellingPrice === undefined
        ? null
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">
            View and manage detailed product master information.
          </p>
        </div>
      </div>
    </div>
            ...
            <div className="md:col-span-2">
              <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-line bg-surface-hover px-4 py-3 transition hover:bg-surface-active">
                <input
                  ...
                  onChange={(event) =>
                    updateEditForm("active", event.target.checked)
                  }
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-muted">
                    Product can be used in active inventory operations.
                  </p>
                </div>
              </label>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-success">
                      Your product information has been saved.
                    </p>
                  </div>
                </div>
              </div>
...
return ( <AppShell> <div className="p-6 lg:p-8"> <div className="rounded-xl border border-line bg-surface px-6 py-16 text-center shadow-sm"> <div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-line border-t-slate-700" /> </div> </div> </AppShell>
);
}

export default function ProductViewPage() {
return (
<Suspense fallback={<ViewPageFallback />}> <ProductViewPageContent /> </Suspense>
);
}
